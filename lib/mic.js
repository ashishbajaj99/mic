...
var mic = function mic(options) {
    ...
    that.start = function start() {
        if(process === null) {
            process = isMac
            ? spawn('rec', ['-b', bitwidth, '--endian', endian, '-c', channels, '-r', rate, '-e', encoding, '-t', 'raw', '-'])
            : spawn('arecord', ['-c', channels, '-r', rate, '-f', format, '-D', device]);
            process.on('exit', function(code, sig) {
                    if(code != null && sig === null) {
                        audioStream.emit('processExitComplete');
                        if(debug) console.log("recording process has exited with code = %d", code);
                    }
                });
            process.stdout.pipe(audioStream);
            if(debug) {
                process.stderr.pipe(infoStream);
            }
