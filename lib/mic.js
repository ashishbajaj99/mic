...
var mic = function mic(options) {
    ...
    // Because arecord supports buffer-size (n samples) arg, but sox supports buffer size (in bytes) arg
    // accept only buffer-size arg and then convert to buffer size for sox
    var bufferBytes = Math.round(Number(bitwidth) / 8 * Number(bufferSize)).toString();

    that.start = function start() {
          if(audioProcess === null) {
            if(isWindows){
              audioProcess = spawn('sox', ['-b', bitwidth, '--endian', endian,
                                   '-c', channels, '-r', rate, '--buffer', bufferBytes,
                                   '-e', encoding, '-t', 'waveaudio', 'default', '-p'],
                                    audioProcessOptions)
            }
            else if(isMac){
              audioProcess = spawn('rec', ['-b', bitwidth, '--endian', endian,
                                    '-c', channels, '-r', rate, '--buffer', bufferBytes,
                                    '-e', encoding, '-t', fileType, '-'], audioProcessOptions)
            }
            else {
              audioProcess = spawn('arecord', ['-t', fileType, '-c', channels, '-r', rate,
                                    '--buffer-size', bufferSize, '-f', format, '-D', device],
                                    audioProcessOptions);
            }
